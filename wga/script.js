...
Presenting	3	gain 15 resource.	lose 15 resource.
...
Forget	3	Shuffle all discarded cards into the deck.	if wasn’t activated, trigger all “Play” affects of all discarded cards.
Stone season	1		"Whenever you’re rolling a dice this turn, roll two dice instead and use the sum as result
"
...
Sponsor	5	"if you have less then 3 resource, gain 11 resource.

"	Lose 3 resource
Trauma	1	gain amount of resource equal to the discarded cards count.	lose resource equal to the discarded cards count.
...
Achive	7	lose 1 resource	If was activated, lose 1 resource.
...
Reversing	3	whenever you would gain any amount of resource this turn, lose this amount of resource instead	whenever you would lose any amount of resource this turn, if that loss wasn't commanded by this card, gain this amount of resource instead
Activating	3	gain 1 resource	if wasn't activated, lose 11 resource
Good management	1	roll a dice. If result is 1, gain 5 resource	lose 7 resource
...
Capitalize	1	whenever you gain resource this turn, instead roll a dice and gain amount of resource equal to the result	lose 5 resource
...
Hi Risk	1	Whenewer you would lose an amount of resource this turn, instead lose twise the amount.	If was activated, gain 7
Privilige	5	if you would lose an amount of resource this turn, loose 1 resource instead	loose 9 resource
Bootstrapping	1	If there are no players with resource count lesser than yours, gain 1 resource and trigger this effect again.	if wasn't activated, lose 3 resource
Trick choice	3	roll two dice. Choose one of the results. Gain amount of resource equal to chosen result. Lose amount of resource equal to result not chosen.	if wasn't activated, lose 3 resource
...
Bartering	1	whenever you would loose an amount of resource this turn, instead choose and discard a card from your hand	lose 5 resource
...
Training	1	when the effect would be triggered this turn, instead trigger same effect but with all numbers replaced by 0	lose 13 resource
...
May indicates choise	1	Show top card of the deck. You may choose to draw that card.	Show top card of the deck. Lose amount of resource equal to the cost of the top card of the deck.
...
Summing costs	5	Choose a number. Reveal an amount of cards equal to the chosen number from the top of the library. If all of the reveal cards share cost, gain amount of resource equal to the sum of costs of all revealed cards.	if wasn't activated, lose 7 resource.
interrogation	1	gain 3 resource	lose an amount of resource equal to activated cards count 
Cooperation is incredulous	5	Choose other player. That player looks at the top card of the library. Choose a number. Reveal a top card of the deck. If cost of the revealed card is equal to the chosen number, gain 7 resource, otherwise chosen player gains 5 resource	Lose 1 resource
Big odds	3	Rool a dice. If the result is equal to or less then 1 gain 7 resource, otherwise lose 1 resource and trigger this effect 	if wasn't activated, lose 3 resource
Sanding	1	Roll a dice and put the result on this card. Whenever you would roll a dice this turn, instead roll a dice and use the result of the roll minus the result on this card as a result.	Roll a dice, lose amount of resource equal to the result
...
Paper season	1	Gain 9 resource. Reveal top card of the deck and put it under this card.	Trigger "play" effect of the card under this card. If you can't, lose 5 resource
...
Probabilition	1	whenever you would roll a dice this turn, instead reveal and discard top card of the deck and use the cost of the revealed card as the result.	Roll a dice. Lose amount of resource equal to the result.
Stone ball	1	whenever you would roll a dice this turn, instead roll a dice then put the result of that roll on this card then use the sum of all results on this cards as the result.	Lose 7 resource. Gain amount of resource equal to the sum of the results on this card.
Confabulation	3	Roll a dice and put the result on this card. Look at the top of the deck. Roll a dice.	If wasn't activated, lose 3 resource
...
Trying	3	roll a dice. You may choose to reveal a card from your hand with cost that is less then the roll result. if you do, activate revealed card without paying it's cost. If you don't reveal a card, trigger this effect again.	Lose 9 resource
...
Pulling	3	whenever you would draw a card this turn, instead reveal the top card of the deck and put it inder this card.	If there no cards under this one, lose 7 resource. Otherwise, for each card under this card trigger its "activate" effect and trigger its "play" effect.
Aversion	1	Whenever an effect of other card would be triggered this turn, roll a dice and if the result is bigger then 1, cancel the effect. 	lose 11 resource
...
Priorities	1	When you would gain an amount of resource this turn, instead roll a dice a number of times equal to the amount.	If was activated, gain 13 resource.
Alling	3	When you would lose an amount of resource this turn instead lose amount af resource equal to your resource count.	If was activated, gain 11 resource. Otherwise, lose 1 resource.
...
interact('.dice')
  .on('hold', function (event) {
    ...
    $(event.currentTarget).append($(`<input type="text" style="
    width: 15px;
">`))
